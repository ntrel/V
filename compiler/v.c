...
#define main__MAC 0
#define main__LINUX 1
#define main__WINDOWS 2
...
bool string_ge(string s, string a) {
  ...
  for (int i = 0; i < s.len; i++) {
    ...
    if ((/*casttt*/ (int)(/*77*/ s.str[i] /*rbyte 0*/)) <
        (/*casttt*/ (int)(/*77*/ a.str[j] /*rbyte 0*/))) {
      /*if*/

      return 0;

    } else if ((/*casttt*/ (int)(/*77*/ s.str[i] /*rbyte 0*/)) >
               (/*casttt*/ (int)(/*77*/ a.str[j] /*rbyte 0*/))) {
      /*if*/
...
void println2(string s) { printf("%.*s\n", s.len, s.str); }
byte *v_malloc(int n) {
...
string map_string_str(map_string m) {
  ...
  for (int tmp27 = 0; tmp27 < tmp26.len; tmp27++) {
    ...
    string tmp28 = tos("", 0);
    bool tmp29 = map_get(m, entry.key, &tmp28);
    if (!tmp29)
      tmp28 = tos("", 0);
...
array_string os__read_lines(string path) { return os__read_file_lines(path); }
array_string os__read_file_into_lines(string path) {
...
void CGen_genln(CGen *g, string s) {
  ...
  if (string_ne(g->cur_line, tos2(""))) {
    ...
    _PUSH(&g->lines, (g->cur_line), tmp3, string);
...
void CGen_start_tmp(CGen *g) {
  ...
  if (g->is_tmp) {
    ...
    os__exit(_STR("start_tmp() already started. cur_line=\"%.*s\"",
...
int CGen_add_placeholder2(CGen *g) {
  ...
  if (g->is_tmp) {
    ...
    v_exit(tos2("tmp in addp2"));
  };

  _PUSH(&g->lines, (tos2("")), tmp10, string);
...
void CGen_set_placeholder2(CGen *g, int pos, string val) {
  ...
  if (g->is_tmp) {
    ...
    v_exit(tos2("tmp in setp2"));
  };
  string tmp11 = val;

  array_set(&/*q*/ g->lines, pos, &tmp11);
}
void CGen_insert_before(CGen *g, string val) {
...
void CGen_register_thread_fn(CGen *g, string wrapper_name, string wrapper_text,
  ...
  array_string tmp12 = g->thread_args;
  ;
  for (int tmp13 = 0; tmp13 < tmp12.len; tmp13++) {
    string arg = ((string *)tmp12.data)[tmp13];
  ...
  _PUSH(&g->thread_args, (struct_text), tmp14, string);

  _PUSH(&g->thread_args, (wrapper_text), tmp15, string);
}
string V_prof_counters(V *c) {
  ...
  array_Type tmp20 = c->table->types;
  ;
  for (int tmp21 = 0; tmp21 < tmp20.len; tmp21++) {
    Type typ = ((Type *)tmp20.data)[tmp21];

    array_Fn tmp22 = typ.methods;
    ;
    for (int tmp23 = 0; tmp23 < tmp22.len; tmp23++) {
      Fn f = ((Fn *)tmp22.data)[tmp23];
...
void Parser_fn_call(Parser *p, Fn f, int method_ph, string receiver_var,
  ...
  bool is_print = p->is_prod && !p->is_test && !p->builtin_pkg &&
                  (/*lpar*/ string_eq(f.name, tos2("println")) ||
                   (/*lpar*/ f.is_c && string_eq(f.name, tos2("printf"))));
...
string vtmp_path() {
  ...
  return string_add(os__home_dir(),
                    _STR("/.vlang%.*s/", main__Version.len, main__Version.str));
}
int main(int argc, char **argv) {
  ...
  if (_IN(string, tos2("version"), args)) {
    ...
    println2(_STR("V %.*s", main__Version.len, main__Version.str));
  ...
  if (array_string_contains(args, tos2("fmt"))) {
    ...
    if (!os__file_exists(file)) {
      ...
      os__exit1(_STR("\"%.*s\" does not exist", file.len, file.str));
    };

    if (!string_ends_with(file, tos2(".v"))) {
      /*if*/

      os__exit1(tos2("v fmt can only be used on .v files"));
    };

    println2(tos2("vfmt is temporarily disabled"));
...
void V_compile(V *c) {
  ...
  if (c->os == main__MAC) {
    ...
    CGen_genln(cgen, tos2("#define mac (1) "));
  };

  if (c->os == main__LINUX) {
    /*if*/

    CGen_genln(cgen, tos2("#define linux (1) "));

    CGen_genln(cgen, tos2("#include <pthread.h>"));
  };

  if (c->os == main__WINDOWS) {
    /*if*/

    CGen_genln(cgen, tos2("#define windows (1) "));

    CGen_genln(cgen, tos2("#include <windows.h> "));
  };
  ...
  CGen_genln(
      ...
      tos2("   \n#include <stdio.h>  // TODO remove all these includes, define "
           ...
           "(type *)memdup((type[]){ __VA_ARGS__ }, sizeof(type)) \n#define "
           "UTF8_CHAR_LEN( byte ) (( 0xE5000000 >> (( byte >> 3 ) & 0x1e )) & "
           "3 ) + 1 \n\n//================================== GLOBALS "
           "=================================*/   \n//int V_ZERO = 0; "
           "\nbyteptr g_str_buf; \nint load_so(byteptr);\nvoid "
           "reload_so();\nvoid init_consts();"));
  ...
  if (imports_json && c->build_mode == EMBED_VLIB ||
    ...
    CGen_genln(cgen, tos2("#include \"json/cJSON/cJSON.c\" "));
  };
  ...
  if (c->build_mode == DEFAULT_MODE) {
    ...
    if (imports_json) {
      ...
      CGen_genln(cgen, tos2("#include \"json/cJSON/cJSON.h\""));
    };
  };
  ...
  if (c->is_test || c->is_run) {
    ...
    if (ret != 0) {
      ...
      os__exit1(tos2("ret not 0, exiting"));
    };
  };
}
void V_cc(V *c) {
  ...
  if (c->build_mode == BUILD) {
    ...
    if (!os__file_exists(libs)) {
      ...
      println2(tos2("`builtin.o` not found"));
  ...
  if (c->os == main__LINUX && !linux_host) {
    /*if*/
  ...
  if (c->os == main__MAC) {
    ...
    _PUSH(&a, (tos2("-mmacosx-version-min=10.7")), tmp35, string);
  };

  _PUSH(&a, (flags), tmp36, string);

  _PUSH(&a, (libs), tmp37, string);

  if (c->os == main__MAC) {
    /*if*/

    _PUSH(&a, (tos2("-x objective-c")), tmp38, string);
  };
  ...
  if (c->os == main__LINUX && c->build_mode != BUILD) {
    /*if*/
  ...
  if (c->os == main__LINUX && !linux_host && c->build_mode != BUILD) {
    /*if*/
...
array_string V_v_files_from_dir(V *c, string dir) {
  ...
  if (!os__file_exists(dir)) {
    ...
    v_panic(_STR("%.*s doesnt exist", dir.len, dir.str));
  };
  ...
  for (int tmp50 = 0; tmp50 < tmp49.len; tmp50++) {
    ...
    if (string_ends_with(file, tos2("_win.v")) && c->os != main__WINDOWS) {
      /*if*/

      continue;
    };

    if (string_ends_with(file, tos2("_lin.v")) && c->os != main__LINUX) {
      /*if*/

      continue;
    };

    if (string_ends_with(file, tos2("_mac.v")) && c->os != main__MAC) {
      /*if*/
...
V *new_v(array_string args) {
  ...
  if (is_script && !os__file_exists(dir)) {
    ...
    v_exit(_STR("`%.*s` does not exist", dir.len, dir.str));
  };
...
array_string run_repl() {
  ...
  println2(tos2("Use Ctrl-D to exit"));

  println2(tos2("For now you have to use println() to print values, this will "
                "be fixed soon\n"));
  ...
  while (1) {
    ...
    if (string_starts_with(line, tos2("print"))) {
      ...
      _PUSH(&lines, (line), tmp121, string);
    };
  };
...
string Parser_dot(Parser *p, string str_typ, int method_ph) {
  ...
  if (has_field) {
    ...
    if (string_eq(field.name, tos2("age"))) {
      /*if*/
...
string Parser_index_expr(Parser *p, string typ, int fn_ph) {
  ...
  Parser_log(&/* ? */ *p, _STR("!!indexer typ=%.*s", typ.len, typ.str));
...
string Parser_array_init(Parser *p) {
  ...
  if (no_copy) {
    ...
    new_arr = string_add(new_arr, tos2("_no_copy"));
  };
  ...
  if (p->tok == IF) {
    ...
    if (_IN(string, name, main__SupportedPlatforms)) {
      ...
      if (not) {
        ...
        Parser_genln(p, _STR("#ifndef %.*s", name.len, name.str));

      } else {
        /*else if*/

        Parser_genln(p, _STR("#ifdef %.*s", name.len, name.str));
      };
...
void Parser_chash(Parser *p) {
  ...
  if (string_starts_with(hash, tos2("flag "))) {
    ...
    if (string_contains(hash, tos2("linux")) && p->os != main__LINUX) {
      /*if*/

      return;

    } else if (string_contains(hash, tos2("darwin")) && p->os != main__MAC) {
      /*if*/
    ...
    _PUSH(&p->table->flags, (flag), tmp256, string);
  ...
  if (string_starts_with(hash, tos2("include"))) {
    ...
    if (Parser_first_run(&/* ? */ *p) && !is_sig) {
      ...
      _PUSH(&p->cgen->includes, (_STR("#%.*s", hash.len, hash.str)), tmp257,
            string);
    ...
    if (Parser_first_run(&/* ? */ *p)) {
      ...
      _PUSH(&p->cgen->typedefs, (_STR("%.*s", hash.len, hash.str)), tmp258,
            string);
    };
    ...
    if (string_starts_with(hash, tos2("ifdef"))) {
      ...
      if (string_eq(os, tos2("linux")) && p->os != main__LINUX) {
        /*if*/
    ...
    if (string_contains(hash, tos2("define"))) {
      ...
      _PUSH(&p->cgen->includes, (_STR("#%.*s", hash.len, hash.str)), tmp262,
            string);
...
void Parser_at(Parser *p) {
  ...
  array_string vals = string_split(p->lit, tos2(" "));

  if (vals.len != 2) {
    /*if*/

    Parser_error(p, tos2("Bad @ syntax (type name)"));
  };

  string typ = (*(string *)array__get(vals, 0));

  if (!Table_known_type(&/* ? */ *p->table, typ)) {
    /*if*/

    Table_register_type(p->table, typ);
  };
...
string Parser_js_decode(Parser *p) {
  ...
  if (string_eq(op, tos2("decode"))) {
    ...
    for (int tmp311 = 0; tmp311 < tmp310.len; tmp311++) {
      Var field = ((Var *)tmp310.data)[tmp311];
    ...
    _PUSH(&p->cgen->typedefs,
          (_STR("typedef Option %.*s;", opt_type.len, opt_type.str)), tmp314,
          string);
...
Scanner *new_scanner(string file_path) {
  ...
  if (!os__file_exists(file_path)) {
    ...
    v_panic(_STR("\"%.*s\" doesnt exist", file_path.len, file_path.str));
  };
...
void Scanner_error(Scanner *s, string msg) {
  ...
  println2(_STR("panic: %.*s:%d", file.len, file.str, s->line_nr + 1));
...
bool Parser_satisfies_interface(Parser *p, string interface_name, string _typ,
  ...
  for (int tmp57 = 0; tmp57 < tmp56.len; tmp57++) {
    ...
    if (!Type_has_method(&/* ? */ *typ, method.name)) {
      ...
      Parser_error(p,
                   _STR("Type \"%.*s\" doesnt satisfy interface \"%.*s\" "
...
string Table_cgen_name(Table *table, Fn *f) {
  ...
  if (table->obfuscate && string_ne(f->name, tos2("main")) &&
    ...
    println2(_STR("%.*s ==> %.*s", old.len, old.str, name.len, name.str));
  };
...
string Table_cgen_name_type_pair(Table *table, string name, string typ) {
  ...
  if (typ.len > 0 && string_at(typ, 0) == '[') {
    ...
    if (string_eq(T->name, tos2(""))) {
      ...
      os__exit1(tos2("this should never happen"));
    };
